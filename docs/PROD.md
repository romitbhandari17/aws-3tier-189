...
| ALB security group | `0.0.0.0/0` on port 80 (HTTP only) | If keeping any public-facing LB: add an **ACM certificate + HTTPS listener (443)**, redirect 80→443, and scope ingress down (e.g. to API Gateway's IP ranges or behind VPC Link so it's not public at all — see §1) |
